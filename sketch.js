...
/*   if (frameCount%500===0)
   ...
   if (gameState==="grassland")
   {
    ...
    if (healthbar.height<5&&gameState==="grassland")
{
    gameState="death";
}
   }
   ...
   if (gameState==="desert")
   {
    ...
    health.y=player.y-300;
    healthbar.y=player.y-300;
       if (healthbar.height<5&&gameState==="desert")
{
    gameState="death";
}
   }
   ...
   if (gameState==="snow")
   {
    ...
    health.y=player.y-300;
    healthbar.y=player.y-300;
       if (healthbar.height<5&&gameState==="snow")
{
    gameState="death";
}
   }
   ...
   if (gameState==="water")
   {
    ...
    health.y=player.y-300;
    healthbar.y=player.y-300;
       if (healthbar.height<5&&gameState==="water")
{
    gameState="death";
}
   }
   ...
   if (gameState==="jungle")
   {
    ...
    health.y=player.y-300;
    healthbar.y=player.y-300;
       if (healthbar.height<5&&gameState==="jungle")
{
    gameState="death";
}
       ...
       if (keyDown("space")&&player.y>-300)
   {
      ...
      // player.velocityX=0;
       ...
       if (healthbar.height<5&&gameState==="space")
{
    gameState="death";
}
...
for (var i = 1;i<ar.length;i++)
   ...
   //     console.log("hsau");
    ...
    if (bullet.x>player.x+75||bullet.x<player.x-75)
    {
        bullet.destroy();
    }
    ...
    for (var i = 0; i < bulletg.length; i++) { if (bulletg.get(i).isTouching(animalgroup)) { bulletg.get(i).destroy(); ee=ee-1 } }
     
    for (var i = 0; i < bulletg.length; i++) { if (bulletg.get(i).isTouching(monster1)) { bulletg.get(i).destroy(); ml=ml-1 } }
...
function spawn_animal()
{
    if (frameCount%300===0)
    {
     ...
     animal.debug=true;
     animal.setCollider("rectangle",0,0,400,400);
     ...
     else if(gameState==="water")
     {
         ...
         animal.scale=0.7;
         animal.setCollider("rectangle",0,0,250,250);
     }
